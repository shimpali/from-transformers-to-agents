...
notebook, the standalone scripts the notebook is built from, and the diagrams as SVGs. Each script
under `scripts/` gets its own GitHub Gist, so a snippet can be embedded right where it's referenced
in the article rather than as one big code dump. The layout follows
...
Medium: pushing each snippet's Gist and generating paste-ready HTML. See
[Publishing with main.py](#publishing-with-mainpy) below.
...
│   └── ...
├── gist/
│   └── GIST.md            one Gist ID + published URL per script, kept in sync by `main.py --sync-gists`
└── medium/
...
- **`--sync-gists`**: pushes **every** `scripts/*.py` file to its **own** Gist through the GitHub
  API -- one Gist per snippet, not one combined Gist for the whole article, so each can be embedded
  on its own wherever it's referenced. First run creates each gist; every run after that updates the
  same ones in place (matched by the IDs stored in `gist/GIST.md`), so re-running after editing a
  script never creates duplicates. GitHub's Gist API still works fine for this, unlike Medium's own
  API, which stopped issuing new integration tokens years ago, so there's no equivalent automation
  for the Medium side.
- **Default, no flag needed**: converts each article's `README.md` into Medium-paste-ready HTML.
  ...
  GitHub first. Each `scripts/<name>.py` link becomes that specific snippet's own Gist URL once it's
  published, with a bare copy of that URL dropped on the next line for the live-embed trick below; any
  tables get flattened into bullet lists (Medium doesn't support tables), and the GitHub-only "In this
  folder" section gets cut. Output: `<article>/medium/<slug>.medium.html`.

Run both together so the HTML always carries each snippet's fresh Gist URL:
...
empty line by itself, not as part of a bigger paste. So after pasting the article, for each snippet
you want live: find the bare Gist URL sitting right after its "Full script: [...]" line, select it,
delete it, and paste that same URL again on its own to get the live embed -- right where that
snippet belongs, instead of one link at the end for the whole article.
